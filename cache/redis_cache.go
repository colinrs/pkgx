...
func NewRedisCache(rc *RedisConfig) RedisCache {
	var pool = &redis.Pool{
		MaxIdle:     rc.Idle,
		MaxActive:   rc.Max,
		...
		Dial: func() (redis.Conn, error) {
			...
			if rc.Passwd != "" {
				if _, err := c.Do("AUTH", rc.Passwd); err != nil {
					c.Close()
					return nil, err
				}
			}
	...
	return RedisCache{pool, rc.DefaultExpiration}
}
...
// RedisItemMapGetter Implement a Getter on top of the returned item map.
